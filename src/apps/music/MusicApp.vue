    ...
    <div v-if="show" class="app-window" style="z-index: 50; background: #111;">
      ...
      <div class="music-header">
        ...
        <div class="header-info">
          <div v-if="musicState.coListenCharId" class="co-listen-badge">
            <i class="fas fa-headphones"></i> 一起听
          </div>
          <div v-else style="font-size:12px; color:#aaa; font-weight:600; letter-spacing:1px;">
            AERO MUSIC
          </div>
        </div>
        <button class="icon-btn" @click="activeModal = 'search'">
      ...
      <div class="main-view" @click="viewMode = viewMode === 'disc' ? 'lrc' : 'disc'">
        ...
        <div class="lrc-wrapper" :class="{ 'hidden': viewMode === 'disc' }">
          <LrcScroller 
            :parsedLyrics="musicState.parsedLyrics"
            :currentLyricIndex="musicState.currentLyricIndex"
            :currentSubtitle="musicState.islandSubtitle"
            @seek="handleSeek"
            @share="openLyricShareTarget"
          />
        </div>
      </div>
      ...
      <div class="control-area">
        ...
        <div class="btn-row">
          ...
          <button class="play-btn" @click="togglePlay">
            <i :class="musicState.isPlaying ? 'fas fa-pause' : 'fas fa-play'"></i>
          </button>
        ...
        <div class="action-row">
          ...
          <button class="action-btn" @click="activeModal = 'library'">
            <i class="fas fa-folder-open"></i> 歌单库
          </button>
          <button class="action-btn" @click="activeModal = 'stats'">
            <i class="fas fa-chart-bar"></i> 记录
          </button>
          <button class="action-btn" @click="openIslandSettings">
            <i class="fas fa-layer-group"></i> 悬浮窗
          </button>
        </div>
      </div>
      ...
      <div class="ios-alert-mask" v-if="!!pendingLrcText && !pendingLrcCharId" @click.self="pendingLrcText = null" style="z-index: 100;">
        <div class="ios-alert" style="width: 280px; padding:0;">
          <div class="ios-alert-title" style="padding:15px; border-bottom:1px solid #eee;">
            发送歌词给谁？
          </div>
          <div style="max-height: 300px; overflow-y:auto; text-align:left;">
             <div v-for="char in characters" :key="char.id" class="char-share-item" @click="pendingLrcCharId = char.id">
               {{ char.name }}
             </div>
          </div>
          <div class="ios-alert-btn" @click="pendingLrcText = null" style="width:100%; border-top:1px solid #eee;">
            取消
          </div>
        </div>
      </div>
      ...
      <div class="ios-alert-mask" v-if="!!pendingLrcCharId" @click.self="pendingLrcCharId = null; pendingLrcText = null" style="z-index: 100;">
        <div class="ios-alert">
          <div class="ios-alert-title">
            <i class="fas fa-quote-left" style="color:#5c8aff;"></i> 分享歌词卡片
          </div>
          <div class="ios-alert-desc">
            <b>《{{ musicState.currentSongName }}》</b><br>
            <i style="margin-top: 5px; display: inline-block;">{{ pendingLrcText }}</i>
          </div>
          <div class="ios-alert-inputs">
            <input class="ios-alert-input" v-model="customLrcMsg" placeholder="添加附加语 (选填)" />
          </div>
      ...
      <div class="ios-alert-mask" v-if="showIslandSettings" @click.self="showIslandSettings = false" style="z-index: 100;">
        <div class="ios-alert" style="width: 300px;">
          ...
          <div class="ios-alert-actions">
            <div class="ios-alert-btn bold" style="width: 100%; border: none;" @click="showIslandSettings = false">完成</div>
          </div>
        </div>
      </div>
...
const openIslandSettings = () => {
  try {
    ...
    if (s) islandSettings.value = JSON.parse(s)
  } catch(e) {}
  showIslandSettings.value = true
}

const saveIslandSettings = () => {
  localStorage.setItem('islandSettings', JSON.stringify(islandSettings.value))
  window.dispatchEvent(new CustomEvent('update-island-settings', { detail: islandSettings.value }))
}

const progressPercent = computed(() => {
  if (musicState.totalDuration === 0) return 0
  return (musicState.currentTime / musicState.totalDuration) * 100
})
...
const onProgressClick = (e) => { 
  ...
  if (musicState.totalDuration) { 
    seek(musicState.totalDuration * ((e.clientX - rect.left) / rect.width)) 
  } 
}

const handleSeek = (time) => seek(time)

const openLyricShareTarget = (txt) => {
...
const confirmShareLrc = () => {
  ...
  pushMessage(targetChatId, { 
    role: 'user', 
    type: 'lyric_share', 
    text: pendingLrcText.value, 
    song: musicState.currentSongName, 
    content: customLrcMsg.value || '这句歌词让我想起了你...' 
  })
  
  window.dispatchEvent(new CustomEvent('sys-toast', { detail: '歌词卡片已发送至聊天' }))
  pendingLrcCharId.value = null
  pendingLrcText.value = null
}
</script>
